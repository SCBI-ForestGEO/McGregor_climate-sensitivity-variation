...
We used Akaike information criterion with correction for small sample sizes (AICc) to assess model selection, and conditional/marginal R-squared to assess model fit as implemented in the AICcmodavg package in R [@R-AICcmodavg]. 
...
Trait variables were considered appropriate for inclusion in the main model if they had a consistent direction of response across all droughts and if their addition to the base model improved fit (at $\Delta$AICc $\ge$ 1.0, where $\Delta$AICc is the difference in AICc between models with and without the trait) in at least one drought year (Table S4). 
...
Taller trees (based on $H$ in the drought year) showed stronger growth reductions during drought (Table 1; Figs. **4, S6**). 
...
$Rt$ had a significantly negative response to $ln[TWI]$ across all drought years combined (Figs. **4, S6**, Table S8-S9). 
...
In contrast, $PLA_{dry}$, and $\pi_{tlp}$ - the traits that qualified for inclusion in the full model (Table S4) - were both negatively correlated to drought resistance (Figs. **4, S6**; Tables 1,S4-S5,S8-S9). 
