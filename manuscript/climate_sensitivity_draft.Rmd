...
The general statistical model for hypothesis testing was a mixed effects model, implemented  in the lme4 package in R [**REF**], using tree nested within species as a random effect, and independent variables as specified below. 
...
These tests followed the model structure specified above, including $ln[H]$ and $TWI$ in the null model.
...
Belowground, taller trees would tend to have larger root systems (**SCALING REF**), but this does not necessarily imply that they have greater access to or reliance on deep soil-water resources that may be critical during drought (**REFS- PANAMA ISOTOPES STUDY**).
Rather, larger trees may allocate more to abundant shallow roots that are beneficial for taking up water from rainstorms (**PANAMA ISOTOPES STUDY**).
