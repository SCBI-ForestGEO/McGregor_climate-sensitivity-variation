...
We test a series of hypotheses and associated specific predictions (Table 1) based on the combination of tree-ring records from three droughts (1966, 1977, 1999), species functional and hydraulic trait measurements, and census data from a large forest dynamics plot in Virginia, USA. 
...
To avoid over-fitting models with five species traits (Table 3) across only 12 species, we did not include all traits as fixed effects in a single GLMM, but rather conducted individual tests of each species trait to determine the relative importance and appropriateness for inclusion in the main model. 
...
To do so, we compared models with all possible combinations of candidate variables, including $ln[H]$*$TWI$ and species traits as specified above.
