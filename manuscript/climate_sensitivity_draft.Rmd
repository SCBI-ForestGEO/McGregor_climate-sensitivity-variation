...
Specifically, we used linear regression on log-transformed data to relate $r_{bark}$ to diameter inside bark from 2008 data (Table S2), which were then used to determine $r_{bark}$ in the $DBH$ reconstruction. 
...
Species-specific height allometries were developed (Table S3) using log-log regression ($ln[H] \sim ln[DBH]$). 
...
The distribution of $H$ across drought years is shown in Fig. **S3**. 
...
While we tested the signficance of xylem porosity as a predictor (Table 1), we did not consider it appropriate for inclusion in the main model because of highly uneven distribution of species across categories (Table 2) and opposite drought responses of the only two diffuse-porous species (detailed below).
...
In presentation of the results below, we note instances where the $Rt_{ARIMA}$ model disagreed with the $Rt$ model, but otherwise do not discuss the $Rt_{ARIMA}$ model. Visualization of the best mixed effects model per drought scenario was created by the visreg package [@R-visreg].
