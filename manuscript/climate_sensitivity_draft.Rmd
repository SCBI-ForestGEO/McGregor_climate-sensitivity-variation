...
kable(hypothesis_table, booktabs = TRUE, caption = "**Table 1. Summary of hypotheses, corresponding specific predictions, and results.** We count predictions as fully supported / rejected when the response matches/contradicts the prediction in both univariate and multivariate models (when applicable). Parentheses indicate that predictions were partially supported/ rejected--i.e., that the direction of response matched/contradicted the prediction in some but not all models.") %>%
  ...
  pack_rows("H1.3. Small trees with lower root volume suffer proportionally more in drier microhabitats.", 6, 6, latex_gap_space = "1em", colnum = 1, hline_before = FALSE) %>%
...
kable(hypothesis_table, booktabs = TRUE, caption = "**Table 2. Summary of variables**") %>%
  ...
  pack_rows("Independent variables", 2, 21, latex_gap_space = "1em", colnum = 1, hline_before = FALSE) %>%
  add_header_above(c(" "," "," "," "," "," ", "observed values" = 3, " ")) %>%
  #kable_styling("striped", full_width = F) %>%
  #add_indent(c(6,7,9,13, 15,16,17,18,19,20,21))
  kable_styling()
```
...
Topographic wetness index (TWI) was calculated using the [@R-dynatopmodel] package in R. [** Ian, include a brief explanation of what this is**]
