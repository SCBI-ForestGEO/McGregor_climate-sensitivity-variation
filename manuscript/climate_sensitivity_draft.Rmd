...
kable(hypothesis_table, booktabs = TRUE, caption = "**Table 1. Summary of hypotheses, corresponding specific predictions, and results.** We count predictions as fully supported / rejected when the response matches/contradicts the prediction in both univariate and multivariate models (when applicable). Parentheses indicate that predictions were partially supported/ rejected--i.e., that the direction of response matched/contradicted the prediction in some but not all models.") %>%
  ...
  pack_rows("H3.1. Overall community drought resistance differed across droughts.", 17, 17, latex_gap_space = "1em", colnum = 1, hline_before = TRUE) %>%
  pack_rows("H3.2. The direction of responses to predictor variables was consistent across droughts.", 18, 19, latex_gap_space = "1em", colnum = 1, hline_before = FALSE) %>%
  pack_rows("H3.3. The strength of responses to predictor variables varied across droughts.", 18, 19, latex_gap_space = "1em", colnum = 1, hline_before = FALSE) %>%
...
We support the hypothesis (*H2.3*) that the observed tendency for larger trees to have greater growth reductions during drought (lower $R$) is *not* driven by more drought-sensitive traits in larger trees, but rather by size itself (Tables 1,5). As discussed above, there was little meaningful variation in traits with height at the community level. When $H$ and hydraulic traits were considered together in multivariate models, the effect of $H$ on $R$ was consistently negative (Table 5)--reversing a non-signficant positive $H$-$R$ correlation in the univariate model for the 1999 drought (Table 4).


*Responses across droughts*

We reject *H3.1*. Within the context of mixed effects models, there were no significant differences in $R$ across drought years (Table 4). This is consistent with the observation that the distribution of $R$ values was similar across droughts (Fig. 1b).

We support Despite the broad similarity in the extent to which the three droughts affected the community as a whole, there was variation as to which variables best predicted drought responses (*H3.2b*), and occasionally in the direction of these responses (*H3.2b*). 
