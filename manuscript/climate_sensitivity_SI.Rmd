...
library(knitr)
library(kableExtra)
singlevar_table <- read.csv("tables_figures/publication/tableS4_singlevariable_models_reform_arimaratio.csv", stringsAsFactors = FALSE, check.names = FALSE) 
kable(singlevar_table, booktabs = TRUE, caption = "Table S4. Single-variable tests of hypothesized drivers of drought resistance, where $Rt_{ARIMA}$ is used as the response variable. Models including each variable were compared to corresponding null models. Delta AICc is the AICc of the null model minus that of the model including the variable (thus, Delta AICc > 1 indicates that the variable significantly improves the model). Variable abbreviations are as in Table 2.", format = 'latex', escape = FALSE) %>%
...
library(knitr)
library(kableExtra)
multivar_table <- read.csv("tables_figures/publication/tableS5_multivariable_models_reform_arimaratio.csv", stringsAsFactors = FALSE, check.names = FALSE) 
kable(multivar_table, booktabs = TRUE, caption = "Table S5. Summary of top full models for each drought instance, where $Rt_{ARIMA}$ is used as the response variable. Models are ranked by AICc, and we show all models whose AICc value falls within 1.0 (Delta AICc<1) of the best model (bold).", format = 'latex', escape = FALSE) %>%
  kable_styling(latex_options = c("scale_down", "hold_position"), font_size = 12, protect_latex = TRUE) %>%
