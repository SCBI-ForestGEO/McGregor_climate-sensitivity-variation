...
library(knitr)
library(kableExtra)
singlevar_table <- read.csv("tables_figures/publication/tableS4_singlevariable_models_lmer_CPout.csv", stringsAsFactors = FALSE, check.names = FALSE) #note: this table is a rearranged version of `tested_traits_all.csv`.
...
library(knitr)
library(kableExtra)
singlevar_table <- read.csv("tables_figures/publication/tableS5_singlevariable_models_lmer_arimaratio_CPout.csv", stringsAsFactors = FALSE, check.names = FALSE) 
...
library(knitr)
library(kableExtra)
multivar_table <- read.csv("tables_figures/publication/table5_multivariable_models_lmer_CP+HmodelsDeleted.csv", stringsAsFactors = FALSE, check.names = FALSE) 
kable(multivar_table, booktabs = TRUE, caption = "Table S6. Summary of top full models for each drought instance, where $Rt$ is used as the response variable. ", format = 'latex', escape = FALSE) %>%
  ...
  pack_rows("", 6, 14, latex_gap_space = ".4em", colnum = 1, hline_before = FALSE) %>%
  pack_rows("", 15, 21, latex_gap_space = ".4em", colnum = 1, hline_before = FALSE) %>%
  row_spec(1, bold = T) %>%
  row_spec(5, bold = T) %>%
  row_spec(6, bold = T) %>%
  row_spec(15, bold = T) %>%
...
library(knitr)
library(kableExtra)
multivar_table <- read.csv("tables_figures/publication/tableS6_multivariable_models_lmer_arimaratio_CP+HmodelsDeleted.csv", stringsAsFactors = FALSE, check.names = FALSE) 
kable(multivar_table, booktabs = TRUE, caption = "Table S7. Summary of top  models for each drought instance, where $Rt_{ARIMA}$ is used as the response variable. ", format = 'latex', escape = FALSE) %>%
  kable_styling(latex_options = c("scale_down", "hold_position"), font_size = 12, protect_latex = TRUE) %>%
  add_header_above(c(" "," "," "," "," ", " ", " ", "crown position" = 4, 
                     " "," ")) %>%
  pack_rows("", 1, 4, latex_gap_space = ".4em", colnum = 1, hline_before = FALSE) %>%
  pack_rows("", 5, 7, latex_gap_space = ".4em", colnum = 1, hline_before = FALSE) %>%
  pack_rows("", 8, 17, latex_gap_space = ".4em", colnum = 1, hline_before = FALSE) %>%
  pack_rows("", 18, 28, latex_gap_space = ".4em", colnum = 1, hline_before = FALSE) %>%
  row_spec(1, bold = T) %>%
  row_spec(5, bold = T) %>%
  row_spec(8, bold = T) %>%
  row_spec(18, bold = T) %>%
...
\newpage
![Figure S3: Height (from reconstructed DBH) by crown position across the three focal droughts and in the year of measurement (2018)](tables_figures/publication/figureS3_height_plot_analysis.png){width=600px}

\newpage
![Figure S4: **PLACEHOLDER** ](tables_figures/publication/independent_variables_by_species.png)


\newpage
![Figure S5: Comparison of $Rt$ and $Rt_{ARIMA}$ results, with residuals, for each drought scenario](tables_figures/publication/figureS4_Rt_arima_comparison.png)

\newpage
![Figure S6: **PLACEHOLDER** ](tables_figures/publication/figureS5_species Rt differences.png)
